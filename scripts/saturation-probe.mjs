...
function contextLines({ fixture, withProject }) {
  return [
    ...
    '  Three ceilings can refuse, and the lowest of them is the knee:',
    '    depth   workers + queueLimit, where workers is CLAMPED to',
    '            availableParallelism() - 1. On a 2-vCPU box that is 1 + 8 = 9,',
    '            not the 2 + 8 = 10 the shipped defaults suggest.',
    '    time    a job waiting longer than queueWaitLimitMs (5 s) is refused',
    '            rather than started. This one moves with how fast the box',
    '            drains, so a slow instance knees BELOW its depth cap.',
    '    memory  parseBudgetBytes / (2 MB + 10 x file size) — 550 MiB / 43 MB',
    '            is ~13 for a 4 MB file, so it only binds for the big ones.',
